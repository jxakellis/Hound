...
const getDogs = async (req, res) => {
    ...
    if (dogId) {
        ...
        try {
            const result = await queryPromise(req, 'SELECT * FROM dogs WHERE dogs.userId = ?',
                [userId])
...
const createDog = async (req, res) => {
    ...
    try {
        const result = await queryPromise(req,
            'INSERT INTO dogs(userId, icon, name) VALUES (?,?,?)',
            [userId, undefined, dogName])
        req.commitQueries(req)
        return res.status(200).json({ message: 'Success', dogId: result.insertId })
    } catch (error) {
        req.rollbackQueries(req)
