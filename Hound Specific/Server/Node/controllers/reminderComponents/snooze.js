...
const createSnoozeComponents = async (req, reminderId) => {
    ...
    if (isSnoozed === true) {
        ...
        await queryPromise(req, 'INSERT INTO reminderSnoozeComponents(reminderId, isSnoozed, snoozeExecutionInterval, snoozeIntervalElapsed) VALUES (?,?,?,?)',
            [reminderId, isSnoozed, snoozeExecutionInterval, snoozeIntervalElapsed])
    }
    else {
        await queryPromise(req, 'DELETE FROM reminderSnoozeComponents WHERE reminderId = ?',
            [reminderId])
    }
}
/**
...
const updateSnoozeComponents = async (req, reminderId) => {
    ...
    if (isSnoozed === true) {
        ...
        try {
            ...
            await queryPromise(req, 'UPDATE reminderSnoozeComponents SET isSnoozed = ?, snoozeExecutionInterval = ?, snoozeIntervalElapsed = ? WHERE reminderId = ?',
