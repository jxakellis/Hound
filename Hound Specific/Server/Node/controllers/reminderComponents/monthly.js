...
const createMonthlyComponents = async (req, reminderId) => {
  ...
  await queryPromise(req, 'INSERT INTO reminderMonthlyComponents(reminderId, hour, minute, dayOfMonth) VALUES (?,?,?,?)',
...
const updateMonthlyComponents = async (req, reminderId) => {
    ...
    try {
        ...
        //If this fails: The components provided are invalid. It is uncaught here to intentionally be caught by invocation from reminders.
         if (skipping === true){
            await queryPromise(req, 'UPDATE reminderMonthlyComponents SET hour = ?, minute = ?, dayOfMonth = ?, skipping = ?, skipDate = ? WHERE reminderId = ?',
            [hour, minute, dayOfMonth, skipping, skipDate, reminderId])
        }
        else {
            await queryPromise(req, 'UPDATE reminderMonthlyComponents SET hour = ?, minute = ?, dayOfMonth = ?, skipping = ?  WHERE reminderId = ?',
