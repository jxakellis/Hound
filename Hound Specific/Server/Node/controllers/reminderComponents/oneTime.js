...
const createOneTimeComponents = async (req, reminderId) => {
   ...
   await queryPromise(req, 'INSERT INTO reminderOneTimeComponents(reminderId, date) VALUES (?,?)',
   [reminderId,date])
}
...
const updateOneTimeComponents = async (req, reminderId) => {
   ...
   [reminderId,date])
        ...
        await queryPromise(req, 'UPDATE reminderOneTimeComponents SET date = ? WHERE reminderId = ?',
     [date,reminderId])
    }
