...
const { alertLogger } = require('../../logging/pino');
...
const createLogNotification = async (userId, familyId, dogId, logAction, logCustomActionName) => {
  ...
  if (areAllDefined([userId, familyId, dogId, logAction, logCustomActionName])) {
    try {
      ...
      if (areAllDefined([user, user.userFirstName, user.userLastName, dog, dog.dogName])) {
      ...
      alertLogger.error(`createLogNotification error ${error}`);
    }
  }
};
