...
class NotificationManager {
    ...
    /**
     DOES update local UserConfiguration. Requests permission to send notifications to the user then invokes updateServerUserNotificationConfiguration. If the server returned a 200 status and is successful, then true is returned. Otherwise, if the user didn't grant permission or there was a problem with the  query, false is returned and (if needed) ErrorManager is automatically invoked
     */
