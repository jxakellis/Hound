...
async function createSubscriptionForUserIdFamilyIdRecieptId(connection, userId, familyId, base64EncodedAppStoreReceiptURL) {
  ...
  const castedBase64EncodedAppStoreReceiptURL = formatBase64EncodedString(base64EncodedAppStoreReceiptURL);

  if (areAllDefined(connection, userId, familyId, castedBase64EncodedAppStoreReceiptURL) === false) {
    throw new ValidationError('connection, userId, familyId, or base64EncodedAppStoreReceiptURL missing', global.constant.error.value.MISSING);
  }
  ...
  const requestBody = {
    ...
    'receipt-data': castedBase64EncodedAppStoreReceiptURL,
  ...
  await updateReceiptRecords(connection, userId, familyId, receipts);

  // get the most recent subscription to return to the user
  return getActiveSubscriptionForFamilyId(connection, familyId);
}
 ...
 */
async function updateReceiptRecords(connection, userId, familyId, receipts) {
  const castedReceipts = formatArray(receipts);
  const subscriptionLastModified = new Date();

  if (areAllDefined(connection, userId, familyId, castedReceipts) === false) {
    throw new ValidationError('connection, userId, familyId, or receipts missing', global.constant.error.value.MISSING);
  }

  // Filter the receipts. Only include one which their productIds are known, and assign values if receipt is valid
  for (let i = 0; i < castedReceipts.length; i += 1) {
    const receipt = castedReceipts[i];
